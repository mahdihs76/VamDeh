...
class ActivationActivity : BaseActivityJobSupport() {
    ...
    private fun activateUser(phone: String, code: String) = showLoading().also { scheduleJob(SendVerificationJob.TAG, getPhoneAndCodeExtras(phone, code)) }
    ...
    @Subscribe(threadMode = ThreadMode.MAIN)
    fun onEvent(event: SendVerificationErrorEvent) = hideLoading().also { toast(event.error) }
